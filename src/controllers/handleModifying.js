...
`;
    ...
    try {
        identificationResponse = await getChatGPTResponse([{role: 'system', content: identificationPrompt}, {role: 'user', content: Body}]);
    ...
    if (identificationResponse.trim().toLowerCase() === 'solicitud de modificación') {
        ...
        openAIResponse = await getChatGPTResponse([{ role: 'user', content: `${promptPedido}, El mensaje es: ${Body}`}]);
